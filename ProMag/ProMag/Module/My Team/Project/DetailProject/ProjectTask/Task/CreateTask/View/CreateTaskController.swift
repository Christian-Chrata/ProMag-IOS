...
class CreateTaskController: UIViewController {
    ...
    private var viewModel = CreateTaskViewModel(roleId: "")
    private var state: CreateTaskModel = .create(projectId: "")
    
    init(viewModel: CreateTaskViewModel, state: CreateTaskModel) {
        super.init(nibName: "CreateTaskController", bundle: Bundle(for: CreateTaskController.self))
    ...
    private func submit() {
        ...
        switch state {
        case .create:
            viewModel.insertTask(with: state.projectId) { (state) in
        ...
        case .edit:
            viewModel.editTask(with: state.taskId) { (state) in
                DispatchQueue.main.async { [weak self] in
                    switch state {
                    case .loading:
                        LoadingController.shared.show()
...
extension CreateTaskController: SelectTeamControllerDelegate {
